/**
 * Generated by orval v7.16.0 🍺
 * Do not edit manually.
 * Ferretería API
 * API REST para gestión de ferretería con soporte multi-tenant.
      
**Características:**
- Multi-tenancy por subdominio
- Autenticación JWT
- Gestión completa de inventario
- Control de caja y ventas
- Integración SUNAT
- Auditoría completa

**Autenticación:**
Todos los endpoints (excepto /auth) requieren token JWT en header Authorization: Bearer <token>
 * OpenAPI spec version: 2.0.0
 */
import type { ListCobranzasQueryAllOfEstado } from "./listCobranzasQueryAllOfEstado";

export type ListCobranzasQueryAllOf = {
  /** Búsqueda por nombre de cliente */
  q?: string;
  /**
   * Filtrar por ID de cliente
   */
  cliente_id?: number;
  /** Filtrar por estado de la cuenta */
  estado?: ListCobranzasQueryAllOfEstado;
  /** Fecha de inicio del rango de vencimiento (ISO 8601) */
  fecha_inicio?: string;
  /** Fecha fin del rango de vencimiento (ISO 8601) */
  fecha_fin?: string;
};
